...
const seed: Task[] = [
  ...
  { id:'t3', title:'Google Sheets 연동 방식 정리', workspace:'데이터 플랫폼', due:'내일', status:'todo', priority:'보통' },
...
export function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="workspace"><header className="topbar"><div><h1>{view === 'personal' ? '오늘 할 일을 정리합니다.' : view === 'group' ? '그룹 업무를 확인합니다.' : '프로젝트 연결을 설정합니다.'}</h1><p>{view === 'projects' ? 'Google Sheets 권한을 확인한 뒤 동기화합니다.' : '필요한 작업만 그룹에 공유합니다.'}</p></div></header>
      {view !== 'projects' && <><div className="timeline-scroll"><section className="timeline"><div className="timeline-head"><span>이번 주</span>{['월','화','수','목','금'].map((day) => <span key={day}>{day}</span>)}</div>{['개인 작업','제품 기획','데이터 플랫폼'].map((lane, laneIndex) => <div className="lane" key={lane}><strong>{lane}</strong><div className="lane-track">{tasks.filter((task) => task.workspace === lane).slice(0,2).map((task,index) => <button key={task.id} onClick={() => setSelectedId(task.id)} className={`timeline-task lane-${laneIndex} start-${index}`}>{task.title}</button>)}</div></div>)}</section></div>
    ...
    <aside className="inspector"><div className="inspector-top"><span>{selected.workspace}</span><button aria-label="패널 닫기">×</button></div><h2>{selected.title}</h2><dl><div><dt>상태</dt><dd>{statusLabel[selected.status]}</dd></div><div><dt>마감</dt><dd>{selected.due}</dd></div><div><dt>담당</dt><dd>김해경</dd></div><div><dt>우선순위</dt><dd>{selected.priority}</dd></div></dl><div className="publish"><h3>그룹에 공유</h3><button onClick={() => setView('group')}>그룹 작업 열기</button></div><div className="activity"><h3>최근 이력</h3><p>개인 작업에 추가됨</p><p>그룹 공유 대기</p></div></aside>
  </div>;
}

function ProjectConnections() { return <section className="connections"><div><h2>연결한 프로젝트</h2><p>Google Sheets 권한을 확인한 뒤 동기화합니다.</p></div><article><h3>팀 업무 보드</h3><p>Google Sheets · 읽기·공유 권한 필요</p><button>권한 확인</button></article><article><h3>새 프로젝트</h3><p>Google Sheets URL을 추가합니다.</p><button>URL 추가</button></article></section>; }
